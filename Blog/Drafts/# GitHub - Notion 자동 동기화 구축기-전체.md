...
### 일괄 동기화 중 발생한 변환 실패 (validation_error)

기존 .md 파일들을 일괄로 동기화하는 `sync-full.js`를 처음 돌렸을 때, 87개 중 3개 파일이 Notion API의 `validation_error`로 실패. 원인은 모두 마크다운 안의 특정 구조가 Notion이 받지 않는 형태로 변환된 것.

**케이스 1: localhost URL을 자동 링크로 감지**

```
백엔드에서 http://localhost:8545로 요청 보내서 ...
```

martian이 평문의 `http://localhost:8545`를 자동 링크로 변환하는데, Notion은 `localhost` URL을 거부 (`Invalid URL for link`).

처리: 백틱으로 감싸서 인라인 코드로 만들기.
```
백엔드에서 `http://localhost:8545`로 요청 보내서 ...
```

**케이스 2: 페이지 내 이동 fragment 링크 (목차)**
...
블로그 글 상단에 자주 쓰는 목차(TOC) 패턴. `#`으로 시작하는 fragment-only 링크는 Notion이 invalid URL로 거부. 9개 링크가 모두 같은 원인.

처리: 목차 부분을 일반 번호 리스트로 변경. Notion에는 `/Table of contents` 블록이 있어 자체적으로 목차 생성 가능하므로 마크다운 목차는 불필요.

**케이스 3: 마크다운 문법 치트시트 파일 (변환 자체가 부적합)**

`Templates/디렉토리 마크다운.md` 같은 마크다운 문법 예시 파일. setext 헤더(`==`, `--`), HTML 블록(`<pre><code>`), 들여쓰기 코드 블록, autolink(`<http://...>`) 등 다양한 문법이 섞여 있어 martian 변환 시 중첩 구조가 어긋남:

```
body.children[19].quote.children[1].quote.children[1].quote.children should be not present
```

이런 파일은 콘텐츠 자체가 "마크다운 문법"이라 Notion으로 변환되면 의도가 손상됨. 처리: 동기화 제외 패턴에 해당 폴더 추가.
...
일괄 동기화에서 실패하는 파일들은 보통 다음 중 하나:
- 평문에 섞인 localhost/내부망 URL
- 페이지 내 이동 목적의 anchor 링크
- 마크다운 문법 자체가 콘텐츠인 파일

새로 글을 쓸 때는 위 패턴을 피하거나, 해당 파일을 동기화 대상에서 빼면 깔끔하다.
