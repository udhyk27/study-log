...
[UI 원칙]
- 화려한 디자인보다 기능과 사용성 우선
- 불필요한 애니메이션 / 라이브러리 최소화
- 필요한 데이터와 API만 사용
- 목록은 페이징 적용
...
- 불필요한 객체 생성과 코드 최소화
- 예외는 GlobalExceptionHandler로 공통 처리


[DB / Query 원칙]
- 필요한 데이터만 조회
- N+1 방지
- 적절한 Fetch Join / DTO Projection 사용
- 목록 조회는 Pagination 적용
- WHERE / JOIN 조건에 필요한 Index 검토
- 대량 데이터는 Batch / Cursor Pagination 검토
- 쿼리 변경 후 EXPLAIN으로 실행계획 확인
- 불필요한 DB 조회 최소화
...
- DB 쿼리 수 / 실행시간 확인
- 불필요한 Redis 캐싱 금지
- 대량 데이터를 한 번에 메모리에 올리지 않는다.
- Thread Pool / Connection Pool 무분별한 증가 금지
- 비동기 처리는 실제 필요한 경우에만 사용
...
- 필요한 경우가 아니면 새로운 라이브러리 추가 금지
- 요구사항에 없는 기능 임의 추가 금지
- 관련 없는 파일 수정 금지
- 가독성과 유지보수성을 우선
