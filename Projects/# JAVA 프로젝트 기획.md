...
각 프로젝트의 **핵심 기술만 명확히 어필**하고, 기능 욕심을 줄여 한 달 내 완성 가능한 규모로 구성.
...
```
주문 요청 → Redis Queue → Worker → MySQL (Batch Insert) → 대시보드
```
...
사용자별 잔액(또는 포인트)을 충전 / 차감 / 조회하는 시스템. 중복 요청 방지와 금액 정합성을 핵심으로 함.
...
| 순서 | 프로젝트 | 기간 | 비고 |
|------|---------|------|------|
| 1 | TicketFlow | 4주 | 동시성 + Queue + 운영 로그 통합 |
| 2 | JProxy | 3~4주 | 차별화 |
| 3 | Mini Payment Ledger | 2주 | 금액 정합성 / Idempotency |
...
- **Mini Payment Ledger**: 금액 정합성 / Idempotency / 감사 로그

---

# 우선순위 진행 순서

1. **TicketFlow 완성** (배포까지)
   - 1~2주차: 회원, 상품, 선착순 구매, 락 비교
   - 3주차: Redis TTL, 운영 로그 Queue + Worker
   - 4주차: 대시보드, 성능 테스트, 배포
2. **JProxy 진행** (차별화 포인트 확보)
3. **Mini Payment Ledger** (2주 안에 빠르게)
